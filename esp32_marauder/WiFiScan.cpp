...
extern "C" {
  ...
  NimBLEAdvertisementData WiFiScan::GetUniversalAdvertisementData(EBLEPayloadType Type) {
    ...
    switch (Type) {
      ...
      default: {
        CSerial.println("Please Provide a Company Type");
        break;
      }
    }
      ...
      void onResult(BLEAdvertisedDevice *advertisedDevice) {
        ...
        if (wifi_scan_obj.currentScanMode == BT_SCAN_ALL) {
          if (buf >= 0)
          {
            ...
            CSerial.print(" RSSI: ");
            CSerial.print(advertisedDevice->getRSSI());

            display_string.concat(" ");
            CSerial.print(" ");

            CSerial.print("Device: ");
            if(advertisedDevice->getName().length() != 0)
            {
              display_string.concat(advertisedDevice->getName().c_str());
              CSerial.print(advertisedDevice->getName().c_str());

            }
            else
            {
              display_string.concat(advertisedDevice->getAddress().toString().c_str());
              CSerial.print(advertisedDevice->getAddress().toString().c_str());
            }
  ...
  class bluetoothScanSkimmersCallback: public BLEAdvertisedDeviceCallbacks {
      void onResult(BLEAdvertisedDevice *advertisedDevice) {
        ...
        if (buf >= 0)
        {
          ...
          CSerial.print(" RSSI: ");
          CSerial.println(advertisedDevice->getRSSI());
        }
      }
  };
#endif
...
int WiFiScan::clearStations() {
  ...
  stations->clear();
  CSerial.println("stations: " + (String)stations->size());
...
int WiFiScan::clearAPs() {
  ...
  CSerial.println("access_points: " + (String)access_points->size());
  return num_cleared;
}
...
int WiFiScan::clearSSIDs() {
  ...
  ssids->clear();
  CSerial.println("ssids: " + (String)ssids->size());
  return num_cleared;
}
...
bool WiFiScan::addSSID(String essid) {
  ...
  ssids->add(s);
  CSerial.println(ssids->get(ssids->size() - 1).essid);
...
/*void WiFiScan::joinWiFi(String ssid, String password)
{
  ...
  if ((WiFi.status() == WL_CONNECTED) && (ssid == connected_network) && (ssid != "")) {
    ...
    CSerial.println("Already connected. Disconnecting...");
    WiFi.disconnect();
  }
  ...
  CSerial.print("Connecting to WiFi");
...
void WiFiScan::initWiFi(uint8_t scan_mode) {
  ...
  if (scan_mode != WIFI_SCAN_OFF) {
    //CSerial.println(F("Initializing WiFi settings..."));
    this->changeChannel();
    ...
    //CSerial.println(F("Initialization complete"));
  }
}
...
void WiFiScan::StopScan(uint8_t scan_mode)
{
  ...
  (currentScanMode == BT_SCAN_SKIMMERS))
  {
    ...
    #endif
    //CSerial.print("display_buffer->size(): ");
    CSerial.println(display_obj.display_buffer->size());
...
void WiFiScan::RunLoadAPList() {
    ...
    if (!file) {
      CSerial.println("Could not open /APs_0.log");
      #ifdef HAS_SCREEN
        display_obj.tft.setTextWrap(false);
    ...
    if (error) {
      CSerial.print("JSON deserialize error: ");
      CSerial.println(error.c_str());
      file.close();
      #ifdef HAS_SCREEN
        display_obj.tft.setTextWrap(false);
        display_obj.tft.setFreeFont(NULL);
    ...
    #endif
    CSerial.print("Loaded APs:");
    CSerial.println((String)access_points->size());
  #endif
}
...
void WiFiScan::RunLoadSSIDList() {
    ...
    if (!log_file) {
      CSerial.println("Could not open /SSIDs_0.log");
      #ifdef HAS_SCREEN
        display_obj.tft.setTextWrap(false);
        display_obj.tft.setFreeFont(NULL);
        display_obj.tft.setCursor(0, 100);
    ...
    CSerial.print("Loaded SSIDs: ");
    CSerial.println((String)ssids->size());
  #endif
}
...
void WiFiScan::RunEvilPortal(uint8_t scan_mode, uint16_t color)
{
  ...
  //  CSerial.println("Could not successfully start EvilPortal. Setting WIFI_SCAN_OFF...");
  //  this->StartScan(WIFI_SCAN_OFF, TFT_MAGENTA);
  //  return;
  //}
  //else
  //  CSerial.println("Setup EvilPortal. Current mode: " + this->currentScanMode);
  this->wifi_initialized = true;
  initTime = millis();
}
...
void WiFiScan::RunAPScan(uint8_t scan_mode, uint16_t color)
{
  ...
  CSerial.println(text_table4[9] + (String)access_points->size());
  #ifdef HAS_SCREEN
    display_obj.TOP_FIXED_AREA_2 = 48;
    display_obj.tteBar = true;
    display_obj.print_delay_1 = 15;
  ...
  void WiFiScan::RunLvJoinWiFi(uint8_t scan_mode, uint16_t color) {
    ...
    display_obj.tft.setRotation(1);
      ...
      CSerial.println("Using TFT Shield");
    #else if defined(TFT_DIY)
      uint16_t calData[5] = { 213, 3469, 320, 3446, 1 }; // Landscape TFT DIY
      CSerial.println("Using TFT DIY");
    #endif
    #ifdef HAS_ILI9341
...
void WiFiScan::RunGPSInfo() {
    ...
    CSerial.println("Refreshing GPS Data on screen...");
    #ifdef HAS_SCREEN
    ...
    // Display to serial
    CSerial.println("==== GPS Data ====");
    if (gps_obj.getFixStatus())
      CSerial.println("  Good Fix: Yes");
    else
      CSerial.println("  Good Fix: No");

    if(text != "") CSerial.println("      Text: " + text);

    CSerial.println("Satellites: " + gps_obj.getNumSatsString());
    CSerial.println("  Accuracy: " + (String)gps_obj.getAccuracy());
    CSerial.println("  Latitude: " + gps_obj.getLat());
    CSerial.println(" Longitude: " + gps_obj.getLon());
    CSerial.println("  Altitude: " + (String)gps_obj.getAlt());
    CSerial.println("  Datetime: " + gps_obj.getDatetime());
  #endif
}
...
void WiFiScan::RunPacketMonitor(uint8_t scan_mode, uint16_t color)
{
  ...
  CSerial.println("Running packet scan...");
  esp_wifi_init(&cfg);
  esp_wifi_set_storage(WIFI_STORAGE_RAM);
  esp_wifi_set_mode(WIFI_MODE_NULL);
  esp_wifi_start();
...
void WiFiScan::RunEapolScan(uint8_t scan_mode, uint16_t color)
{
  ...
  if (err != 0)
  {
    CSerial.print("could not set protocol : err=0x");
    CSerial.println(err, HEX);
  }
  ...
  if (err != 0)
  {
    CSerial.print("AP config set error, Maurauder SSID might visible : err=0x");
    CSerial.println(err, HEX);
  }
    ...
    if (gps_obj.getGpsModuleStatus()) {
      ...
      if (n > 0) {
        for (int i = 0; i < n; i++) {
          ...
          CSerial.print((String)this->mac_history_cursor + " | " + wardrive_line);
...
void WiFiScan::RunBluetoothScan(uint8_t scan_mode, uint16_t color)
{
    ...
    CSerial.println("Started BLE Scan");
    this->ble_initialized = true;
    initTime = millis();
  #endif
}
  ...
  if (type == WIFI_PKT_MGMT)
  {
    ...
    if ((snifferPacket->payload[0] == 0x80) && (buf == 0))
    {
      ...
      if (src == "de:ad:be:ef:de:ad") {
        ...
        delay(random(0, 10));
        CSerial.print("RSSI: ");
        CSerial.print(snifferPacket->rx_ctrl.rssi);
        CSerial.print(" Ch: ");
        CSerial.print(snifferPacket->rx_ctrl.channel);
        CSerial.print(" BSSID: ");
        CSerial.print(addr);
        ...
        {
          CSerial.print((char)snifferPacket->payload[i + 38]);
          //display_string.concat((char)snifferPacket->payload[i + 38]);
          if (isAscii(snifferPacket->payload[i + 38]))
            essid.concat((char)snifferPacket->payload[i + 38]);
          else
            CSerial.println("Got non-ascii character: " + (String)(char)snifferPacket->payload[i + 38]);
        }
        //essid.concat("\": \"\"}}");
...
void WiFiScan::apSnifferCallbackFull(void* buf, wifi_promiscuous_pkt_type_t type) {
  ...
  if (type == WIFI_PKT_MGMT)
  {
    ...
    if ((snifferPacket->payload[0] == 0x80) && (buf == 0))
    {
      ...
      for (int i = 0; i < access_points->size(); i++) {
        mac_match = true;
        //CSerial.print("Checking ");
        //CSerial.print(addr);
        //CSerial.println(" against " + (String)access_points->get(i).essid);


        for (int x = 0; x < 6; x++) {
          //CSerial.println((String)snifferPacket->payload[x + 10] + " | " + (String)access_points->get(i).bssid[x]);
          if (snifferPacket->payload[x + 10] != access_points->get(i).bssid[x]) {
            mac_match = false;
            //CSerial.println("MACs do not match");
            break;
          }
        }
        if (mac_match) {
      ...
      if (!in_list) {
        ...
        CSerial.print(access_points->size());
        CSerial.print(" ");
        CSerial.print(esp_get_free_heap_size());
      ...
      if (wifi_scan_obj.currentScanMode == WIFI_SCAN_SIG_STREN) {
        ...
        for (int i = 0; i < access_points->size(); i++) {
          if (access_points->get(i).selected) {
            ...
            if (found) {
              //CSerial.println("Received beacon from " + access_points->get(i).essid + ". Checking RSSI...");
        ...
        if ((targ_ap.rssi + 5 < snifferPacket->rx_ctrl.rssi) || (snifferPacket->rx_ctrl.rssi + 5 < targ_ap.rssi)) {
          ...
          CSerial.println((String)access_points->get(targ_index).essid + " RSSI: " + (String)access_points->get(targ_index).rssi);
          return;
        }
      }
        ...
        buffer_obj.append(snifferPacket, len);
          ...
          if (gps_obj.getGpsModuleStatus()) {
            ...
            if (do_save) {
              if (n > 0) {
                for (int i = 0; i < n; i++) {
                  CSerial.printf("%-32.32s", WiFi.SSID(i).c_str());
                  CSerial.print(" -> ");
                  CSerial.println(wifi_scan_obj.security_int_to_string(WiFi.encryptionType(i)).c_str());
                }
              }
...
void WiFiScan::stationSnifferCallback(void* buf, wifi_promiscuous_pkt_type_t type) {
  ...
  CSerial.print((String)stations->size() + ": ");

  char sta_addr[] = "00:00:00:00:00:00";

  if (ap_is_src) {
    CSerial.print("ap: ");
    CSerial.print(ap_addr);
    CSerial.print(" -> sta: ");
    getMAC(sta_addr, snifferPacket->payload, 4);
    CSerial.println(sta_addr);
  }
  else {
    CSerial.print("sta: ");
    getMAC(sta_addr, snifferPacket->payload, 10);
    CSerial.print(sta_addr);
    CSerial.print(" -> ap: ");
    CSerial.println(ap_addr);
  }
...
void WiFiScan::rawSnifferCallback(void* buf, wifi_promiscuous_pkt_type_t type)
{
  ...
  if (wifi_scan_obj.currentScanMode == WIFI_SCAN_SIG_STREN) {
    ...
    if ((targ_ap.rssi + 5 < snifferPacket->rx_ctrl.rssi) || (snifferPacket->rx_ctrl.rssi + 5 < targ_ap.rssi)) {
      targ_ap.rssi = snifferPacket->rx_ctrl.rssi;
      access_points->set(targ_index, targ_ap);
      CSerial.print((String)access_points->get(targ_index).essid + " RSSI: " + (String)access_points->get(targ_index).rssi);
  ...
  if (type == WIFI_PKT_MGMT)
  {
    ...
    if ((snifferPacket->payload[0] == 0x40) && (buf == 0))
    {
      if (wifi_scan_obj.currentScanMode == WIFI_SCAN_PROBE) {
        ...
        CSerial.print(" Ch: ");
        CSerial.print(snifferPacket->rx_ctrl.channel);
        CSerial.print(" Client: ");
        char addr[] = "00:00:00:00:00:00";
        getMAC(addr, snifferPacket->payload, 10);
        CSerial.print(addr);
        display_string.concat(addr);
        CSerial.print(" Requesting: ");
        display_string.concat(" -> ");
        for (int i = 0; i < snifferPacket->payload[25]; i++)
        {
          CSerial.print((char)snifferPacket->payload[26 + i]);
    ...
    int temp_len = display_string.length();
      ...
      //CSerial.print(" ");
        ...
        if (display_obj.display_buffer->size() == 0)
        {
          ...
          display_obj.display_buffer->add(display_string);
          display_obj.loading = false;
          CSerial.println(display_string);
        }
      #endif
    #endif
...
void WiFiScan::eapolSnifferCallback(void* buf, wifi_promiscuous_pkt_type_t type)
{
  ...
  if (( (snifferPacket->payload[30] == 0x88 && snifferPacket->payload[31] == 0x8e)|| ( snifferPacket->payload[32] == 0x88 && snifferPacket->payload[33] == 0x8e) )){
    num_eapol++;
    CSerial.println("Received EAPOL:");
    ...
    if (wifi_scan_obj.currentScanMode == WIFI_SCAN_ACTIVE_LIST_EAPOL) {
      ...
      for (int i = 0; i < access_points->size(); i++) {
        if (access_points->get(i).selected) {
          ...
          if (found) {
            CSerial.println("Received beacon from " + access_points->get(i).essid + ". Deauthenticating...");
            break;
          }
        }
      }
  ...
  //  for (x_pos = (11 + x_scale); x_pos <= 320; x_pos += x_scale) //go along every point on the x axis and do something, start over when finished
    for (x_pos = (11 + x_scale); x_pos <= 320; x_pos = x_pos)
    {
      ...
      for (uint8_t b = 0; b < BUTTON_ARRAY_LEN; b++)
      {
        ...
        {
          CSerial.println("Bro, key pressed");
          //do_break = true;
        }
        ...
        if (display_obj.key[b].justReleased())
        {
          ...
          if (b == 4) {
            if (set_channel > 1) {
              CSerial.println("Shit channel down");
              set_channel--;
              delay(70);
          ...
          // Channel + button pressed
            ...
            if (set_channel < MAX_CHANNEL) {
              CSerial.println("Shit channel up");
              set_channel++;
              delay(70);
              display_obj.tft.fillRect(127, 0, 193, 28, TFT_BLACK);
              display_obj.tftDrawChannelScaleButtons(set_channel);
            ...
            CSerial.println("Exiting packet monitor...");
      ...
      if (currentTime - initTime >= (GRAPH_REFRESH * 5)) {
        ...
        if (y_pos_x >= HEIGHT_1) {
          CSerial.println("Max EAPOL number reached. Adjusting...");
          num_eapol = 0;
        }
  ...
  //  for (x_pos = (11 + x_scale); x_pos <= 320; x_pos += x_scale) //go along every point on the x axis and do something, start over when finished
    for (x_pos = (11 + x_scale); x_pos <= 320; x_pos = x_pos)
    {
      ...
      if (currentTime - initTime >= GRAPH_REFRESH) {
        //CSerial.println("-----------------------------------------");
        //CSerial.println("Time elapsed: " + (String)(currentTime - initTime) + "ms");
        x_pos += x_scale;
        initTime = millis();
        ...
        //CSerial.println("num_beacon: " + (String)num_beacon);
        //CSerial.println("num_deauth: " + (String)num_deauth);
        //CSerial.println(" num_probe: " + (String)num_probe);
...
void WiFiScan::main(uint32_t currentTime)
{
  ...
  else if (currentScanMode == WIFI_SCAN_ACTIVE_EAPOL)
  {
    ...
    {
      initTime = millis();
      //CSerial.print("packets/sec: ");
      //CSerial.println(packets_sent);
      String displayString = "";
      String displayString2 = "";
      displayString.concat(text18);
      displayString.concat(packets_sent);
